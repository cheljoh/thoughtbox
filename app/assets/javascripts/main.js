...
function getLinks(){
  ...
  $.getJSON('api/v1/links', function(links){
    $.each(links, function(index, link){
      if (link.read){
        buttonReadContent = "<button id=read-" + link.id + " class='unread-link btn'> Mark as Unread </button>";
        linkStyle = "color:#0000FF";
        readOrUnread = "Already perused it";
      }
      else {
        buttonReadContent = "<button id=read-" + link.id + " class='read-link btn'> Mark as Read </button>";
